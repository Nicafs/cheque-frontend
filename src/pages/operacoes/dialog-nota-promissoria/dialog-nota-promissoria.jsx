...
import { PDFViewer as Viewer  , PDFDownloadLink } from '@react-pdf/renderer';
...
function DialogNotaPromissoria({operacao, open, handleClose, flgRelatorio}) {
  ...
  return (
    <Dialog fullWidth={true} maxWidth="xl" open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
      ...
      <DialogContent>
        { !checkMobile() ? 
          <Viewer style={{width: '100%', height: '1200px'}}>
              {flgRelatorio === 1 ? 
          ...
          :
          flgRelatorio === 1 ? 
            <PDFDownloadLink document={<NotaPromissoria operacao={operacao}/>} fileName={`${operacao?.id || 0}_notaPromissoria.pdf`}>
              {({ blob, url, loading, error }) => {
                  ...
                  :
                  count > 2 ?
                  <PDFReader url={url} /> : 'Loading Documento...';
                }
              }
            </PDFDownloadLink>
            :
            <PDFDownloadLink document={<OperacaoCredito operacao={operacao}/>} fileName={`${operacao?.id || 0}_operacaoCredito.pdf`}>
              {({ blob, url, loading, error }) => {
                count ++;
                return loading ?
                'Loading Documento...'
              ...
              // (loading ? 'Loading Documento...' : 'Faça Download da Operação de Crédito!')
              }
            </PDFDownloadLink>
         }
      </DialogContent>
